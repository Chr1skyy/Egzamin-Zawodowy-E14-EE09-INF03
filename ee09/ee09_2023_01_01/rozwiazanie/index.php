...
<body>
    ...
    <div id="prawy">
        ...
        $q = "SELECT pracownicy.id, stanowiska.nazwa, stanowiska.opis FROM pracownicy JOIN stanowiska ON pracownicy.stanowiska_id = stanowiska.id WHERE pracownicy.id = 2;";
        $res = mysqli_query($con, $q);
        while ($row = mysqli_fetch_array($res)) {
            echo "<h4>$row[1]</h4>
            <h5>$row[2]</h5>";
